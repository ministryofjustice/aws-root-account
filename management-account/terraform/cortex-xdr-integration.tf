...
resource "aws_secretsmanager_secret" "cortex_xdr_uuids" {
...
resource "aws_secretsmanager_secret_version" "cortex_xdr_uuids" {
  secret_id = aws_secretsmanager_secret.cortex_xdr_uuids.id
...
resource "aws_cloudformation_stack_set" "cortex_xdr_stack_set" {
  ...
  name         = "cortex-xdr-cloud-app-stack-set"
  parameters = {
    CortexXDRRoleName = "CortexXDRCloudApp",
    ExternalID        = sensitive(random_uuid.cortex_xdr_stack.result)
  }
...
resource "aws_cloudformation_stack_set_instance" "cortex_xdr_stack_set" {
  deployment_targets {
    organizational_unit_ids = [aws_organizations_organization.default.roots[0].id]
  }
