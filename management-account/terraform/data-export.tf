...
resource "aws_bcmdataexports_export" "mojap_data_production_cur_v2_report" {
  export {
    name = "MOJAP-DATA-PRODUCTION-CUR-V2-HOURLY"
    data_query {
      query_statement = <<-EOF
        SELECT 
          bill_bill_type, bill_billing_entity, bill_billing_period_end_date, bill_billing_period_start_date, 
    ...
    destination_configurations {
      s3_destination {
        s3_bucket = "arn:aws:s3:::mojap-data-production-coat-cur-reports-v2-hourly"
        s3_prefix = "moj-cost-and-usage-reports"
        s3_region = "eu-west-2"
        s3_output_configurations {
          overwrite   = "OVERWRITE_REPORT"
